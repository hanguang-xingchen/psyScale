...
- 最后一题时"下一题"变为"提交"
- 顶部进度条显示当前位置
- 桌面端右侧可折叠**圆形答题卡**，已答题绿色可跳转回顾，当前题粉色高亮，未答题灰色不可点
...
- PC 端：左右分栏
  ...
  - 右侧：SVG 雷达图（hover 数据点或标签显示悬浮详情浮层）
- 移动端：上下堆叠
- 下方：维度明细表格
...
- 同心圆参考线 + 分值标签
- 数据多边形半透明填充
- **hover 交互**：浮动信息浮层，显示维度名、分数与等级（颜色匹配该维度）
  - 浮层自动跟随鼠标，溢出时翻转方向避免遮挡
  - 同时高亮对应数据点（圆点放大、多边形描边加粗）
  - 轴标签 hover 同样触发
- 外圈仅显示维度名称，无永久分数标签，保持图表干净
- `viewBox` 自适应容器宽度
